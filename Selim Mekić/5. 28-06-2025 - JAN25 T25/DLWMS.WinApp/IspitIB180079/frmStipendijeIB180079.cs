    ...
    public partial class frmStipendijeIB180079 : Form
    {
        ...
        private void btnDodaj_Click(object sender, EventArgs e)
        {
            if (Validiraj())
            {
                ...
                if (stipendijeGodine.Exists(x => x.StipendijaId == stipendija.Id && x.Godina == godina))
                {
                    MessageBox.Show($"Stipendija {stipendija} već postoji evidentirana na godini {godina}", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
        ...
        {
            // 1. dio
            // -- thread , await async i task ili thread , thread.start
            // -- validacija
            // -- sve vezano za combo box

            //await Task.Run(() => GenerisiStudentiStipendije());

            Thread thread = new Thread(() => GenerisiStudentiStipendije());
            thread.Start();


        }

        private void GenerisiStudentiStipendije()
        {
            // 2. dio 
            // -- kalukacije
            // -- sleep
