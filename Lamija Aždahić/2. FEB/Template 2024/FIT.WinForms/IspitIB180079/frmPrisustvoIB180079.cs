    ...
    public partial class frmPrisustvoIB180079 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Validiraj())
            {
                ...
                {
                    MessageBox.Show("Student je već evidentiran na toj nastavi", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    //                 7 == 7 
        ...
        private void GenerisiPrisustva(Student? student)
        {
            ...
            for (int i = 0; i < sveNastaveOveProstorije.Count(); i++)
            {
                for (int j = 0; j < broj; j++)
                {
                    ...
                    {
                        StudentId = student.Id,
                        NastavaId = sveNastaveOveProstorije[i].Id,
                    };

                    db.PrisustvoIB180079.Add(novoPrisustvo);
                    db.SaveChanges();

                    info += $"{DateTime.Now.ToString("MM.dd HH:mm:ss")} -> {student} za {sveNastaveOveProstorije[i]} {Environment.NewLine}";
            ...
            Action action = () =>
            {
                ...
                UcitajPrisustva();
                MessageBox.Show($"Generisali ste {broj} prisustava za svaku nastavu u odabranoj prostoriji", "Informacija", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtInfo.Text = info;
