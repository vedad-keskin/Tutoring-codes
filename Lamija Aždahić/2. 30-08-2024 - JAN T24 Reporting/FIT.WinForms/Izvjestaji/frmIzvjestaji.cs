...
namespace FIT.WinForms.Izvjestaji
{
    public partial class frmIzvjestaji : Form
    {
        ...
        private void frmIzvjestaji_Load(object sender, EventArgs e)
        {
            ...
            var sviGradovi = db.GradoviIB180079
                .Include(x=> x.Drzava)
            ...
            for (int i = 0; i < sviGradovi.Count(); i++)
            {

                var Red = tblGradovi.NewdsGradoviRow();

                Red.Rb = (i + 1).ToString();
                Red.Grad = sviGradovi[i].Naziv;
                Red.Drzava = sviGradovi[i].Drzava.Naziv;
                Red.Aktivan = sviGradovi[i].Status == true ? "DA" : "NE";
            ...
            rpc.Add(new ReportParameter("Broj", sviGradovi.Count().ToString()));
